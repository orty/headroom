...
# Opt-in "this proxy is a dedicated Copilot ingress" switch. When enabled,
# otherwise-unclassified traffic defaults to the Copilot upstream instead of the
# OpenAI host. Needed because Copilot Chat's connectivity ping (`GET /_ping`) and
# some `/agents/*` control calls carry NO Copilot markers, so per-request
# detection alone routes them to OpenAI and 404s the ping. Off by default, so
# shared multi-provider proxies are unaffected; Anthropic/Claude Code traffic is
# still classified before this fallback applies.
_INGRESS_MODE_ENV = "HEADROOM_COPILOT_INGRESS"
...
def copilot_ingress_enabled() -> bool:
    """Return True when the proxy is configured as a dedicated Copilot ingress."""
    return os.environ.get(_INGRESS_MODE_ENV, "").strip().lower() in ("1", "true", "yes", "on")
