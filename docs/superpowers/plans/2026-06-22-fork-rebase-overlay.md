...
- Keep-set workflows (the only ones that run on the fork): `docker.yml`,
  `deploy-to-aca.yml`, `sync.yml`, `alpha-release.yml`, `mirror-upstream-release.yml`.
- Feature branches replayed by the rebuild MUST be linear (no merge commits).
...
KEEP=(docker.yml deploy-to-aca.yml sync.yml alpha-release.yml mirror-upstream-release.yml)
