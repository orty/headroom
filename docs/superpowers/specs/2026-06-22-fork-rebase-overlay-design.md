...
| PR head branch (e.g. `claude/...`) (fork) | the actual upstream-PR head branch named in the manifest; may contain merge commits — the rebuild linearizes it on the fly (`--no-merges --right-only --cherry-pick`, preferring the `origin/` tracking ref). No separate pre-linearized branch. | maintained by user via the PR |
